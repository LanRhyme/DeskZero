...
; Fixed: Use standard InstallDir without PLACEHOLDER hack
; This ensures Browse button works correctly
...
; 静态编译注册表先前安装路径恢复，避免使用运行时 SHCTX 导致编译报错
!if "${INSTALLMODE}" == "perMachine"
  InstallDirRegKey HKLM "${MANUPRODUCTKEY}" ""
!else
...
; 3. Install mode (only for "both" mode)
!if "${INSTALLMODE}" == "both"
  !define MUI_PAGE_CUSTOMFUNCTION_PRE SkipIfPassive
